...
//指定我们要自定义编辑器的脚本 
[CustomEditor(typeof(ProcessButton))]
...
{
    //对应我们在ProcessButton中创建的字段
    private SerializedProperty _is_process;
    //PS:需要注意一点，使用SerializedProperty 必须在MyButton类_newNumber字段前加[SerializeField]
    private SerializedProperty _process_timer;
    
    protected override void OnEnable()
    {
        base.OnEnable();
        _is_process = serializedObject.FindProperty("isProcess");
        _process_timer = serializedObject.FindProperty("process_timer");
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.PropertyField(_is_process);//显示我们创建的属性
        EditorGUILayout.PropertyField(_process_timer);//显示我们创建的属性
